...
现在让我们把用更具象的东西来体现这种模糊的恐惧感。

也许动物有用......

如果把第三方代码比作一个条杜宾犬。尽管它看起来平静，温柔。但是在它那黑色，明亮的眼睛里，有一种未知的闪烁。我只想说，我不会把我珍爱的东西放在它的附近。

用户的敏感信息描可以看做一只可爱的小仓鼠。我看着它无辜地舔着它的小前脚，梳理它愚蠢的小脸，小仓鼠完全没有注意到杜宾犬，并在杜宾犬面前随意嬉戏。

如果你曾经养过杜宾犬（强烈推荐），你可能知道他们是美妙，温和的生物，不应该得此恶名。尽管如此，我相信你依旧会同意，让小仓鼠和杜宾犬独处是个坏主意。
...
当用户填完表格时，他们会点击提交，并重定向到结账流程的下一个页面。这可能需要进行一些后端验证，以处理们在页面中提交的数据。

为了让这个文件保持漂亮和苗条，我使用原生的表单验证，而不是JavaScript ，而且由于required和pattern属性，达到JavaScript验证般的体验需要花费很大的精力。

[示例在这里](https://codepen.io/davidgilbertson/pen/OzdEbL)，如果你想看到它的实际效果。
...
上面的要点本身并没有实际用处（正如眼尖的读者发现的那样），真正的黑客可能不会像这样追求Express。我只是说明了你的服务器是很简陋的，并且那里运行的任何东西都有机会窃取用户在浏览器中输入的数据。
...
但是，您真的想要创建一个不包含任何第三方代码的表单，却给了第三方代码修改代码的机会吗？这取决于你。
...
我想知道这七个新包的作用是什么？我想知道管理Firebase工具的人是否知道这七个新软件包的作用是什么？我想知道是否有人知道他们的包装需要什么依赖？

## 3. Webpack

您可能已经注意到我没有建议您将“安全”HTML文件合并到构建通道中（例如，共享CSS），即使这样可以解决代码重复问题。
...
同样，我认为这些中的任何一个都不太可能将脚本注入到您压缩后的代码中。但是，如果花费巨大的精力去制作一个原始的，手写的，人类可读的仓鼠友好的HTML文件是错误的，只是在睡觉前用几百个杜宾犬处理它。
...
在我离开之前，我想谈谈过去几周我听到的很多声音 - 建议开发人员应该使用更少的npm包。

我理解这背后的原因：第三方包可能是恶意的，更少的第三方包装意味着更加安全。

但这是一个糟糕的建议： 即使您使用了较少的 npm包，但是您的安全性并不能得到保障。
...
我不在乎是否有一千个包，或者它们会不断变化，或者我永远不会确定其中一个包含恶意代码。
